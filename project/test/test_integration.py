...
テストケース: IT-01 〜 IT-08、UT-BE-16〜18（notifyDropBlocked）
...
# ============================================================
# UT-BE-16〜18: BackendBridge.notifyDropBlocked 統合テスト
# ============================================================
# MC/DC観点 (notifyDropBlocked):
#   C1: D&D 操作発生 → True → DROP_BLOCKED
#   C2: D&D 実行後に通常操作へ影響しないこと
# ============================================================

class TestNotifyDropBlockedIntegration:
    """UT-BE-16〜18 BackendBridge.notifyDropBlocked"""

    def test_UT_BE_16_notify_drop_blocked_returns_drop_blocked(self, qt_app):
        """D&D 実行時は DROP_BLOCKED が返る"""
        from backend_bridge import BackendBridge
        from file_service import FileService
        ...
        result = _parse(bridge.notifyDropBlocked())

        assert result["success"] is False
        assert result["data"] is None
        assert result["error"] == "DROP_BLOCKED"

    def test_UT_BE_17_notify_drop_blocked_does_not_change_base_path(self, qt_app, tmp_path: Path):
        """D&D 通知後も base_path が変更されない"""
        from backend_bridge import BackendBridge
        from file_service import FileService

        fs = FileService()
        fs.set_base_path(str(tmp_path))
        before = fs._base_path
        bridge = BackendBridge(fs)

        _ = _parse(bridge.notifyDropBlocked())

        assert fs._base_path == before

    def test_UT_BE_18_notify_drop_blocked_then_normal_read_works(self, qt_app, tmp_path: Path):
        """D&D 通知後も通常の readFile が影響なく動作する"""
        from backend_bridge import BackendBridge
        from file_service import FileService
        ...
        target = tmp_path / "normal.md"
        target.write_text("# normal", encoding="utf-8")

        drop_res = _parse(bridge.notifyDropBlocked())
        read_res = _parse(bridge.readFile(str(target)))

        assert drop_res["error"] == "DROP_BLOCKED"
        assert read_res["success"] is True
        assert read_res["data"]["content"] == "# normal"

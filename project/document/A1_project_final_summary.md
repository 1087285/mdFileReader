...
| 判定 | 根拠 |
|------|------|
| **Conditional Go（条件付き可）** | Python 層全機能 pytest 49件 PASS。Windows 実機受入テスト（ACC-OK-01～15, ACC-NG-01～09）の完了をリリース最終条件とする。 |
...
| 2 | `git tag v1.3.0 && git push origin v1.3.0` 実行 → GitHub Actions で `mdFileReader.exe` 自動生成 | 08 エージェント（自動） | ⚠️ 受入テスト完了後に実行 |
...
| 工程 | 成果物 | 状態 |
|------|--------|------|
| 工程1. 要件定義 | `project/document/01_requirements.md` | ✅ 承認済み（2026-03-03）　v1.3.0 |
| 工程2. 基本設計 | `project/document/02_basic_design.md` | ✅ 承認済み（2026-03-03）　v1.3.0 |
| 工程3. 詳細設計 | `project/document/03_detailed_design.md` | ✅ 承認済み（2026-03-03）　v1.3.0 |
| 工程4. 実装 | `project/document/04_implementation.md` + `project/src/` | ✅ 承認済み（2026-03-03）　v1.3.0 |
| 工程5. 単体評価 | `project/document/05_unit_test.md` | ✅ 承認済み（2026-03-03）　v1.3.0 |
| 工程6. 結合評価 | `project/document/06_integration_test.md` | ✅ 承認済み（2026-03-03）　v1.3.0 |
| 工程7. システム評価 | `project/document/07_system_test.md` | ✅ 承認済み（2026-03-03）　v1.3.0 |
...
| 入力要件（REQ-IN） | 5 | 1（REQ-IN-05: D&D 禁止応答のバックエンド確認） | 4（GUI 確認要） |
| 出力要件（REQ-OUT） | 5 | 部分（data 値検証） | 5（GUI 確認要） |
| 運用要件（REQ-OPS） | 4 | 0 | 4 |
| 保守要件（REQ-MNT） | 4 | 3 | 1（REQ-MNT-02: README 更新） |

**pytest 実績: 49 / 49 PASS（単体 25、結合 24）**
...
| v1.2.0 | 2026-03-03 | D&D 許可機能追加（全工程 v1.2.0 根拠更新） |
| v1.3.0 | 2026-03-03 | D&D 禁止仕様反映（全工程 v1.3.0 根拠更新） |

# 07 システム評価

## 1. 文書情報

| 項目 | 内容 |
|------|------|
| 文書名 | システム評価報告書 |
| 版数 | v1.3.0 |
| 作成日 | 2026-03-02 |
| 最終更新日 | 2026-03-03（v1.3.0 D&D 禁止仕様反映） |
| 作成者 | GitHub Copilot（07_system_test_agent） |
| 参照元 | `project/document/01_requirements.md` v1.3.0、`project/document/06_integration_test.md` v1.3.0 |
| ステータス | 承認済み（2026-03-03） |

---

## 2. 目的

要件定義書（01）v1.3.0 に記載されたすべての要件について、実装・評価証跡の整合を確認し、リリース可否を判定する。

---

## 3. 評価観点

| 評価区分 | 対象 | 実施方法 |
|---------|------|---------|
| Python 層機能確認 | REQ-PROC-01〜12、REQ-ERR-01〜10（UI依存を除く） | 自動（pytest 単体・結合） |
| GUI / E2E 確認 | REQ-IN-01〜05、REQ-OUT-01〜05、REQ-UI-01〜10、REQ-OPS-01〜04 | 手動（Windows 実機） |
| 受入テスト | ACC-OK-01〜15、ACC-NG-01〜09 | 手動（Windows 実機） |

---

## 4. 要件トレーサビリティ

### 4.1 入力要件

| 要件ID | 要件名 | pytest 証跡 | 実機確認 | 判定 |
|--------|--------|------------|---------|------|
| REQ-IN-01 | フォルダ指定 | — | IT-09d / ACC-OK-01 | ⚠️ 手動確認要 |
| REQ-IN-02 | ファイル選択 | — | ACC-OK-02 | ⚠️ 手動確認要 |
| REQ-IN-03 | テキスト入力 | — | ACC-OK-03 | ⚠️ 手動確認要 |
| REQ-IN-04 | 保存操作 | IT-03/03a/03b | ACC-OK-04, ACC-OK-05 | ⚠️ GUI 確認要 |
| REQ-IN-05 | D&D 入力禁止 | UT-BE-16〜18（unit/integration） | ACC-OK-14, ACC-OK-15 | ⚠️ GUI 確認要 |

### 4.2 処理要件

| 要件ID | 要件名 | pytest 証跡 | 実機確認 | 判定 |
|--------|--------|------------|---------|------|
| REQ-PROC-01 | ツリー取得 | UT-BE-03/04, IT-01 | — | ✅ 確認済み |
| REQ-PROC-02 | パスバリデーション | UT-BE-01/02, IT-07 × 5 | — | ✅ 確認済み |
| REQ-PROC-02a | D&D イベント拒否 | UT-BE-16/17 | ACC-OK-14, ACC-NG-08 | ⚠️ GUI 確認要 |
| REQ-PROC-02b | D&D 無影響保証 | UT-BE-17/18 | ACC-OK-15, ACC-NG-09 | ⚠️ GUI 確認要 |
| REQ-PROC-02c | D&D 通知 | UT-BE-16/17 | ACC-OK-14 | ⚠️ GUI 確認要 |
| REQ-PROC-03 | 読み込み | UT-BE-05/07, IT-02 | — | ✅ 確認済み |
| REQ-PROC-04 | 文字コード自動判定（chardet） | UT-BE-06 | — | ✅ 確認済み |
| REQ-PROC-04a | Shift-JIS 必須対応（cp932 正規化） | UT-BE-06（encoding==cp932） | — | ✅ 確認済み |
| REQ-PROC-05 | 上書き保存（読み込み時エンコードで保存） | UT-BE-08/08a, IT-03/03a | — | ✅ 確認済み |
| REQ-PROC-06 | BOM なし UTF-8（UTF-8 ファイルの場合） | UT-BE-08（BOM バイト検証） | — | ✅ 確認済み |
| REQ-PROC-07 | 新規ファイル作成 | UT-BE-10/11, IT-04, IT-08 | — | ✅ 確認済み |
| REQ-PROC-08 | ファイル削除 | UT-BE-12/13, IT-05, IT-08 | — | ✅ 確認済み |
| REQ-PROC-09 | 名前変更 | UT-BE-14/15, IT-06, IT-08 | — | ✅ 確認済み |
| REQ-PROC-10 | フォルダ外アクセス禁止 | UT-BE-02, IT-07 × 5 | — | ✅ 確認済み |
| REQ-PROC-11 | QWebChannel 連携 | — | IT-09a〜d | ⚠️ 手動確認要 |
| REQ-PROC-12 | HTTP サーバー不使用 | コードレビュー（Flask 等不使用確認済み） | — | ✅ 確認済み |

### 4.3 出力要件

| 要件ID | 要件名 | pytest 証跡 | 実機確認 | 判定 |
|--------|--------|------------|---------|------|
| REQ-OUT-01 | ファイルツリー表示 | IT-01（JSON データ検証） | ACC-OK-01 | ⚠️ GUI 確認要 |
| REQ-OUT-02 | エディタ表示 | IT-02（content 値検証） | ACC-OK-02 | ⚠️ GUI 確認要 |
| REQ-OUT-03 | プレビュー表示 | — | ACC-OK-03 | ⚠️ 手動確認要 |
| REQ-OUT-04 | 保存完了通知 | IT-03（success 値検証） | ACC-OK-04 | ⚠️ GUI 確認要 |
| REQ-OUT-05 | エラー通知 | UT-BE-02/04/07/09/11/13/15/16（error コード検証） | ACC-NG-01〜09 | ⚠️ GUI 確認要 |

### 4.4 画面要件

| 要件ID | 要件名 | pytest 証跡 | 実機確認 | 判定 |
|--------|--------|------------|---------|------|
| REQ-UI-01 | 分割レイアウト | — | IT-10 | ⚠️ 手動確認要 |
| REQ-UI-02 | CodeMirror エディタ | — | IT-10 | ⚠️ 手動確認要 |
| REQ-UI-03 | プレビュー（marked.js） | — | ACC-OK-03 | ⚠️ 手動確認要 |
| REQ-UI-04 | 表示切替トグル | — | IT-10 | ⚠️ 手動確認要 |
| REQ-UI-05 | 保存ボタン | — | ACC-OK-05 | ⚠️ 手動確認要 |
| REQ-UI-06 | Ctrl+S 保存 | — | ACC-OK-04 | ⚠️ 手動確認要 |
| REQ-UI-07 | フォルダ選択ダイアログ | — | IT-09d | ⚠️ 手動確認要 |
| REQ-UI-08 | HTML リソース同梱 | setup_resources.py 実行済み（全 JS/CSS 配置確認） | — | ✅ 確認済み |
| REQ-UI-09 | D&D 無効化 | UT-BE-16〜18（バックエンド側の禁止応答） | ACC-OK-14, ACC-NG-08 | ⚠️ GUI 確認要 |
| REQ-UI-10 | D&D 禁止表示 | UT-BE-16（`DROP_BLOCKED` 応答） | ACC-OK-14 | ⚠️ GUI 確認要 |

### 4.5 非機能要件（運用）

| 要件ID | 要件名 | pytest 証跡 | 実機確認 | 判定 |
|--------|--------|------------|---------|------|
| REQ-OPS-01 | スタンドアロン動作 | REQ-PROC-12 確認済み | `.exe` 実機起動 | ⚠️ 実機確認要 |
| REQ-OPS-02 | 即時起動 | — | `.exe` ダブルクリック起動確認 | ⚠️ 実機確認要 |
| REQ-OPS-03 | 終了操作 | — | ウィンドウ閉じ確認 | ⚠️ 手動確認要 |
| REQ-OPS-04 | インストール不要 | — | 任意フォルダからの起動確認 | ⚠️ 実機確認要 |

### 4.6 保守要件

| 要件ID | 要件名 | 状態 |
|--------|--------|------|
| REQ-MNT-01 | ソースコード管理 | ✅ GitHub リポジトリ `1087285/mdFileReader` で管理中 |
| REQ-MNT-02 | README 更新 | ⚠️ リリース工程（08）で更新予定 |
| REQ-MNT-03 | 工程成果物管理 | ✅ `project/document/01〜07` として Markdown 管理済み |
| REQ-MNT-04 | エージェント連携 | ✅ 工程1〜7 の Markdown 成果物連携で実施済み |

---

## 5. 受入テスト計画（実機確認手順）

> ⚠️ 以下は Windows 実機（Windows 10 / 11）で実施する手動確認手順。  
> PyInstaller でビルドした `.exe` を使用すること。

### 5.1 正常系受入テスト

| 受入ID | 確認手順 | 期待結果 | 状態 |
|--------|---------|---------|------|
| ACC-OK-01 | アプリ起動 → フォルダ選択 → `.md` ファイルを含むフォルダを選択 | ツリーに `.md` ファイルが表示される | ⚠️ 未実施 |
| ACC-OK-02 | ツリーのファイルをクリック | エディタとプレビューにファイル内容が表示される | ⚠️ 未実施 |
| ACC-OK-03 | エディタで文字を入力または変更する | プレビューがリアルタイムで更新される | ⚠️ 未実施 |
| ACC-OK-04 | 編集後に `Ctrl+S` を押す | ステータスバーに「保存しました」が 3 秒表示され、ファイルが読み込み時のエンコードで保存される | ⚠️ 未実施 |
| ACC-OK-05 | 編集後に保存ボタンを押す | ACC-OK-04 と同等の動作 | ⚠️ 未実施 |
| ACC-OK-06 | ツリーで右クリック → 新規ファイル → 名前入力 | 新規 `.md` ファイルが作成されツリーに反映される | ⚠️ 未実施 |
| ACC-OK-07 | ツリーでファイルを右クリック → 削除 → 確認ダイアログで OK | ファイルが削除されツリーから消える | ⚠️ 未実施 |
| ACC-OK-08 | ツリーでファイルを右クリック → 名前変更 → 新名前入力 | ファイル名が変更されツリーに反映される | ⚠️ 未実施 |
| ACC-OK-09 | `.exe` ファイルをダブルクリック | インストール不要でアプリウィンドウが起動する | ⚠️ 未実施 |
| ACC-OK-10 | Shift-JIS エンコードの `.md` ファイルをツリーから選択 | 文字化けなく内容が表示される | ⚠️ 未実施 |
| ACC-OK-11 | Shift-JIS ファイルを編集して保存する（Ctrl+S） | ファイルが Shift-JIS エンコードで保存される | ⚠️ 未実施 |
| ACC-OK-12 | UTF-8 ファイルを開いて編集・保存する | 保存後も UTF-8 エンコードが維持される | ⚠️ 未実施 |
| ACC-OK-13 | Shift-JIS ファイルに絵文字を入力して保存する | `ENCODE_SAVE_ERROR` が表示されファイルは上書きされない | ⚠️ 未実施 |
| ACC-OK-14 | 任意の `.md` ファイルをアプリウィンドウに D&D する | `DROP_BLOCKED` に対応する禁止メッセージが表示され、ファイルは開かれない | ⚠️ 未実施 |
| ACC-OK-15 | D&D 実行を繰り返す | ルートフォルダ・選択ファイル・エディタ内容・プレビュー内容が操作前と同一である | ⚠️ 未実施 |

### 5.2 異常系受入テスト

| 受入ID | 確認手順 | 期待結果 | 状態 |
|--------|---------|---------|------|
| ACC-NG-01 | 削除済みフォルダ（または存在しないパス）を選択した場合 | エラーメッセージが表示され、アプリがクラッシュしない | ⚠️ 未実施 |
| ACC-NG-02 | 読み取り専用 `.md` ファイルを選択して保存を試みる | 「アクセスが拒否されました」等のエラーが表示される | ⚠️ 未実施 |
| ACC-NG-03 | フォルダ外パスへのアクセスを試行する | `PATH_TRAVERSAL` が返りアクセスが拒否される | ✅ pytest 確認済み（IT-07） |
| ACC-NG-04 | 空ファイル（0 バイト）をツリーから選択 | クラッシュせずエディタが空で表示される | ⚠️ 未実施 |
| ACC-NG-05 | ファイルを右クリック → 名前変更 → 既存ファイル名を入力 | 上書き確認ダイアログが表示される | ⚠️ 未実施 |
| ACC-NG-06 | ファイルを編集後（未保存）に別ファイルをクリック | 未保存変更の警告表示が維持される | ⚠️ 未実施 |
| ACC-NG-07 | ファイルを右クリック → 削除 → 確認ダイアログでキャンセル | ファイルが削除されない | ⚠️ 未実施 |
| ACC-NG-08 | 任意拡張子ファイルをアプリウィンドウに D&D する | 読み込み処理が開始されず、禁止通知のみ表示される | ⚠️ 未実施 |
| ACC-NG-09 | D&D 操作を繰り返す | ツリー更新・ファイル切替・内容変化が発生しない | ⚠️ 未実施 |

---

## 6. 判定

### 6.1 pytest 自動テスト判定

| 区分 | ケース数 | PASS | FAIL | 判定 |
|------|---------|------|------|------|
| 単体テスト（UT） | 25 | 25 | 0 | ✅ OK |
| 結合テスト（IT） | 24 | 24 | 0 | ✅ OK |
| **合計** | **49** | **49** | **0** | **✅ OK** |

### 6.2 自動テストで確認済みの要件

| 確認済み要件 | 証跡 |
|------------|------|
| REQ-PROC-01〜10, REQ-PROC-12 | pytest 49 件 PASS |
| REQ-PROC-02a〜02c（バックエンド禁止応答） | UT-BE-16〜18（unit/integration） |
| REQ-ERR-09〜10（バックエンド状態保証） | UT-BE-16〜18（DROP_BLOCKED / 状態不変） |
| REQ-UI-08（HTML リソース同梱） | setup_resources.py 実行済み（qwebchannel.js, CodeMirror, marked.min.js 配置確認） |
| REQ-MNT-01, REQ-MNT-03, REQ-MNT-04 | GitHub リポジトリ・工程成果物確認済み |

### 6.3 手動確認待ち要件（Windows 実機）

| 要件グループ | 内容 |
|------------|------|
| REQ-IN-01〜04 | フォルダ指定・ファイル選択・テキスト入力・保存操作の GUI 動作 |
| REQ-OUT-01〜05 | ツリー・エディタ・プレビュー・通知の画面表示 |
| REQ-UI-01〜07 | レイアウト・エディタ・プレビュー・トグル・ボタン・ショートカット・ダイアログ |
| REQ-OPS-01〜04 | スタンドアロン動作・起動・終了・インストール不要 |
| REQ-PROC-11 | QWebChannel JS↔Python コールバック連携 |
| REQ-IN-05, REQ-PROC-02a〜02c, REQ-UI-09〜10 | D&D 禁止表示と状態不変の実機確認 |
| ACC-OK-01〜15 | 正常系受入テスト全件 |
| ACC-NG-01〜09（-03 除く） | 異常系受入テスト（-03 は pytest 確認済み） |

### 6.4 リリース可否判定

| 判定 | 条件 |
|------|------|
| **条件付き可** | Python 層（バックエンド）: **PASS 確認済み**。Windows 実機 GUI 受入確認（§5.1/§5.2）完了をリリース条件とする。 |

> **リリース条件:** Windows 10 または Windows 11 実機で `mdFileReader.spec` による PyInstaller ビルドを行い、§5.1（ACC-OK-01〜15）および §5.2（ACC-NG-01〜09）の手動確認が全件 OK であること。

---

## 7. 不具合一覧

| 不具合ID | 対象 | 概要 | 状態 |
|---------|------|------|------|
| BUG-BE-01 | `file_service.py` | PermissionError メッセージ非正規化（工程5で発見・是正済み） | ✅ 是正済み |
| BUG-IT-01 | `backend_bridge.py` | QFileDialog トップレベル import による headless 環境エラー（工程6で発見・是正済み） | ✅ 是正済み |

---

## 8. 工程ゲート（完了承認確認）

| # | 確認項目 | 状態 |
|---|----------|------|
| 1 | 全要件に対するトレーサビリティが作成されている | ✅ 確認済み（§4） |
| 2 | pytest 自動テスト 49 件が全 PASS している | ✅ 確認済み（§6.1） |
| 3 | 手動確認待ち要件が明示されリリース条件として記録されている | ✅ §6.3/6.4 に記録済み |
| 4 | 発見不具合が全て是正済みである | ✅ BUG-BE-01, BUG-IT-01 是正済み |
| 5 | リリース工程（08）への引継ぎ事項が整理されている | ✅ §9 に記録済み |
| 6 | GitHub 使用者のレビュー承認が完了している | ✅ 承認済み（2026-03-03） |

---

## 9. リリース工程への引継ぎ事項

| # | 引継ぎ項目 | 詳細 |
|---|------------|------|
| 1 | Windows 実機確認（必須） | §5 の受入テスト全件実施後にリリース判定を確定すること |
| 2 | PyInstaller ビルド手順 | `project/src/mdFileReader.spec` を使用してビルドする。`setup_resources.py` を事前実行すること |
| 3 | README.md 更新 | `REQ-MNT-02` に従い、起動方法・機能説明・制約事項を README に記載すること |
| 4 | リリースノート作成 | v1.3.0 の変更内容・制約・既知課題を記載すること |
| 5 | 既知制約 | フォルダ作成は `.gitkeep` ファイル経由（createFolder スロット未実装）。次バージョンで追加推奨 |

---

## 10. 差分開発情報

### 10.1 変更一覧

| 変更種別 | 対象 | 内容 |
|----------|------|------|
| 新規作成 | 本文書全体 | `06_integration_test.md` v1.0.0 をもとに初版を新規作成 |

### 10.2 v1.1.0 変更内容（Shift-JIS 対応）

| 変更種別 | 対象 | 内容 |
|----------|------|------|
| トレーサビリティ追加 | §4.2 REQ-PROC-04a | Shift-JIS 必須対応（cp932 正規化）を要件トレースに追加 |
| トレーサビリティ変更 | §4.2 REQ-PROC-05 | 保存要件を「読み込み時エンコードで保存」に更新し UT-BE-08a/IT-03a を証跡に追加 |
| 受入テスト追加 | §5.1 ACC-OK-11〜13 | Shift-JIS/UTF-8 保存維持、`ENCODE_SAVE_ERROR` 表示確認を追加 |
| pytest 件数更新 | §6.1 | UT 22→24、IT 19→21、合計 41→45 |

### 10.3 v1.2.0 変更内容（D&D 許可対応）

| 変更種別 | 対象 | 内容 |
|----------|------|------|
| トレーサビリティ追加 | §4.1 / §4.2 / §4.4 | D&D 関連要件（許可系）を追加 |
| 受入テスト追加 | §5.1 / §5.2 | D&D 正常系・異常系受入を追加 |
| pytest 件数更新 | §6.1 | UT 24→32、IT 21→26、合計 45→58 |

### 10.4 v1.3.0 変更内容（D&D 禁止仕様反映）

| 変更種別 | 対象 | 内容 |
|----------|------|------|
| 参照更新 | §1 文書情報 | 参照元を `01_requirements.md` v1.3.0、`06_integration_test.md` v1.3.0 に更新 |
| トレーサビリティ変更 | §4.1 / §4.2 / §4.4 | D&D 要件を「許可」から「禁止（通知＋状態不変）」へ変更 |
| 受入テスト変更 | §5.1 ACC-OK-14〜15 | D&D 受入期待結果を「表示成功」から「禁止通知・状態不変」へ変更 |
| 受入テスト変更 | §5.2 ACC-NG-08〜09 | D&D 異常受入を「非対応拡張子/文字化け」から「誤読み込み防止/状態変化防止」へ変更 |
| pytest 件数更新 | §6.1 | UT 32→25、IT 26→24、合計 58→49 |
| 判定更新 | §8 | 工程ゲート承認状態をレビュー待ちへ更新 |

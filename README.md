...
| 名前変更 | ツリー右クリック → 「名前変更」 |
| 文字コード自動判定 | UTF-8 / Shift-JIS 等を chardet で自動判定。Shift-JIS は Windows 互換（cp932）に正規化して読み込み || D&D ファイル開封 | `.md` ファイルをアプリウィンドウにドラッグ＆ドロップすると、ツリー選択と同等の操作でファイルを開く。Shift-JIS ファイルも文字化けなく表示 |
---
